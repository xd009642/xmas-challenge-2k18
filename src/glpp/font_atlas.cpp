...
glp::font_atlas::font_atlas(FT_Face face, size_t size) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, MAX_WIDTH, height, 0, GL_RED,
                 GL_UNSIGNED_BYTE, init.data());
    ...
    for(char i = MIN_CHAR; i <= MAX_CHAR; i++) {
        ...
        in.tx1 = static_cast<GLfloat>(w + in.width) /
                 static_cast<GLfloat>(MAX_WIDTH);
        in.ty0 = static_cast<GLfloat>(h + in.height + MARGIN_WIDTH) /
